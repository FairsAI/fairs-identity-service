...
class AuthService {
    ...
    /**
     * 🔒 DEPRECATED: JWT token generation removed - use auth service instead
     * All authentication tokens should be generated by the dedicated auth service
     */
    generateToken(payload, options = {}) {
        // JWT generation removed from identity service - auth service is now the single token authority
        logger.error('DEPRECATED: JWT generation called on identity service', {
            userId: payload.userId,
            notice: 'Use auth service /api/auth/login endpoint instead'
        });
        throw new Error('JWT generation deprecated - use auth service for authentication tokens');
        
        /* REMOVED: JWT generation logic moved to auth service
        try {
            const now = Math.floor(Date.now() / 1000);
    ...
    /**
     * 🔒 DEPRECATED: Guest token generation removed - use auth service instead
     */
    createGuestToken(email) {
        // Guest token generation removed - use auth service for all authentication
        logger.error('DEPRECATED: Guest token generation called on identity service', {
            email: email,
            notice: 'Use auth service for guest authentication instead'
        });
        throw new Error('Guest token generation deprecated - use auth service');
        
        /* REMOVED: Use auth service instead
