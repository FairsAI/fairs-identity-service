...
class AuthService {
    ...
    /**
     * 🔒 SECURITY HARDENED: Generate secure JWT token
     */
    generateToken(payload, options = {}) {
        try {
            ...
            const tokenPayload = {
                ...
                iat: now,
                exp: now + (options.expiresIn || 24 * 60 * 60), // 24 hours default
                iss: 'fairs-identity-service',
                aud: 'fairs-platform',
                jti: crypto.randomUUID() // Unique token ID
            };

            // Sign token with security options
    ...
    /**
     * Create secure guest token for privacy requests
     */
    createGuestToken(email) {
        ...
        }, { expiresIn: 3600 }); // 1 hour for guest tokens
    }
}
